...
from operations.serializers import (
    DailyCensusBatchSerializer, RegionSerializer,
    WeeklyMenuBatchSerializer, WeeklyMenuSerializer,
)
...
class RegionSerializerTest(OpsSerializerTestBase):
    ...
    def test_duplicate_name_rejected(self):
        s = RegionSerializer(data={"name": "East Wing"}, context={"company_id": self.company.id})
        self.assertFalse(s.is_valid())

    def test_different_company_accepted(self):
        co2 = ClientCompany.objects.create(name="Other", code="OTHER01")
        s = RegionSerializer(data={"name": "East Wing"}, context={"company_id": co2.id})
...
class DailyCensusBatchSerializerTest(OpsSerializerTestBase):
    def test_valid_batch(self):
        ...
        data = {"date": "2026-03-01", "items": [
            {"region_id": self.region.id, "diet_category_id": self.diet.id, "count": 50},
            {"region_id": self.region.id, "diet_category_id": diet2.id, "count": 10},
        ]}
    ...
    def test_duplicate_pair_rejected(self):
        data = {"date": "2026-03-01", "items": [
            {"region_id": self.region.id, "diet_category_id": self.diet.id, "count": 50},
            {"region_id": self.region.id, "diet_category_id": self.diet.id, "count": 30},
        ]}
        s = DailyCensusBatchSerializer(data=data)
        self.assertFalse(s.is_valid())
    ...
    def test_negative_count_rejected(self):
        data = {"date": "2026-03-01", "items": [
            {"region_id": self.region.id, "diet_category_id": self.diet.id, "count": -5},
        ]}
        s = DailyCensusBatchSerializer(data=data)
        self.assertFalse(s.is_valid())
...
class WeeklyMenuBatchSerializerTest(OpsSerializerTestBase):
    def test_batch_with_plain_ids(self):
        data = {"menus": [{"company": self.company.id, "diet_category": self.diet.id,
                           "day_of_week": 1, "meal_time": "L",
                           "dishes": [self.dish1.id, self.dish2.id]}]}
        s = WeeklyMenuBatchSerializer(data=data)
        self.assertTrue(s.is_valid(), s.errors)
        menus = s.save()
        self.assertEqual(WeeklyMenuDish.objects.filter(menu=menus[0]).count(), 2)

    def test_batch_with_objects(self):
        data = {"menus": [{"company": self.company.id, "diet_category": self.diet.id,
                           "day_of_week": 2, "meal_time": "B",
    ...
    def test_batch_update_replaces_dishes(self):
        menu = WeeklyMenu.objects.create(
            company=self.company, diet_category=self.diet, day_of_week=3, meal_time="D")
